...
!      real(kind(0.0d0)), parameter :: cutOff = 3d0*(26d0/7d0)**(1d0/6d0)
      ...
      real(kind(0.0d0)) :: Energy(1:nBin), AvgR(1:nBin)
...
!        LJ = 1d0/AvgR(i)**6
!        LJ = 4d0*50d0*LJ*(LJ-1d0)
        LJ = q_tab(ii,jj)*1.671d5/AvgR(i)
        ApproxEnergy(i) = LJ
      enddo 

      
      do i = 1,nBin
        write(6,*) i, cnt(i),AvgR(i),Energy(i)/300d0, (cnt(i)*ApproxEnergy(i))/300d0
