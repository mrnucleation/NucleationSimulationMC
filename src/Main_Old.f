...
!      use MPI
      ...
      real(dp) :: max_dist, max_dist_single
...
!      on the specific role of each function see the comments in the corresponding subfunction.
      call ReadParameters(seed,ncycle,ncycle2,outFreq_Traj,
...
!      write(nout, *) out1
      write(outFormat1, out1) "(", "2x,I9", 
     &                        (",2x,I5",i=1,nMolTypes),
     &                        ",E17.6,2x",
     &                        (",2x,F9.2",i=1,4), ")"
     
      write(outFormat2, out1) "(", "2x,I9", 
     &                        (",2x,I5",i=1,nMolTypes),
     &                        ",F17.4,2x",     
     &                        (",2x,F9.2",i=1,4), ")"     
...
!      write(nout,*) "OpenMP threads:", omp_get_max_threads()
      ...
      write(nout,*) "Initial Energy:", E_T/outputEConv,
     &                   outputEngUnits
      ...
      write(nout,*) "------------------------------------------------"
      write(nout,*) "Cycle # ", "Particles ",  "Energy ",
     &      "Acceptance Rates"
      atmp_1=0.00001d0
      atmp_2=0.00001d0      
      atmp_3=0.00001d0
...
!           write(2,*) E_Torsion_T
         ...
         if(mod(indx,100) .eq. 0 ) then
           call AdjustMax(acc_1,atmp_1,max_dist,dist_limit)
!           call AdjustMax(acc_2,atmp_2,max_dist_single, 0.1d0)
           call AdjustMax(acc_4,atmp_4,max_rot,rot_limit)          
         endif      
        ...
        if(mod(indx,outFreq_GCD) .eq. 0) then
          if(mod(indx,outFreq_Screen) .eq. 0) then
            if(abs(E_T) .lt. 1d6) then
             write(nout,outFormat2) indx,NPART, E_T,1d2*acc_1/atmp_1,
     &                       1d2*acc_3/atmp_3
            else
             write(nout,outFormat1) indx,NPART, E_T,1d2*acc_1/atmp_1,
     &                       1d2*acc_3/atmp_3
            endif
          endif
...
!      in checking for errors that may occur during the simulation. 
      ALLOCATE (FinalNeighborList(1:maxMol,1:maxMol),
     &            STAT = AllocateStatus)
      ALLOCATE (FinalETable(1:maxMol),
     &            STAT = AllocateStatus)     
      ALLOCATE (FinalNeiETable(1:maxMol),
     &            STAT = AllocateStatus)      
...
!     Check for errors in the energy calculation. 
      ...
      if(check .gt. 1d-6) then
        ...
        write(nout,*) "Energy Disagreement"
          write(nout,*) "Culmative Energy:",E_T/outputEConv,
     &                   outputEngUnits
          write(nout,*) "=========================================="
          write(35,*) "=========================================="
      ...
      write(nout,*) "Final Displacement", max_dist      
      write(nout,*) "Acceptance Rate Trans:", 1d2*acc_1/atmp_1
      ...
      write(nout,*) "Cluster Criteria Rejections:", 
     &                 1d2*clusterCritRej/atmp_3      
      if(atmp_3 .ge. 1d0) then
        write(nout,*) "Acceptance Swap In (Mol Type):", 
     &                (1d2*acptSwapIn(j)/atmpSwapIn(j),j=1,nMolTypes)
        write(nout,*) "Acceptance Swap Out (Mol Type):", 
     &                (1d2*acptSwapOut(j)/atmpSwapOut(j),j=1,nMolTypes) 
      ...
      do i=1,maxMol      
        if(isActive(i)) then
          if(FinalETable(i) .ne. 0d0) then
            if(abs((FinalETable(i) - ETable(i))/FinalETable(i))
     &      .gt. 1d-6) then
      ...
      enddo      
!      write(35,*) "Neighbor Energy Table:"
!      do i=1,maxMol
!        if(isActive(i)) then      
!         if(FinalNeiETable(i) .ne. 0d0) then
!           if(abs((FinalNeiETable(i) - NeiETable(i))/FinalNeiETable(i))
!     &        .gt. 1d-6) then
!            write(35,*) i,NeiETable(i),FinalNeiETable(i),"<---Nei Error"
!           else 
!             write(35,*) i, NeiETable(i)
!           endif
!         else
!           if(NeiETable(i) .ne. 0d0) then
!            write(35,*) i,NeiETable(i),FinalNeiETable(i),"<---Nei Error"            
!           else
!            write(35,*) i,NeiETable(i)
!           endif
...
!            zcm = zcm + MolArray(iType)%mol(1)%z(iAtom)
            xcm = xcm + gasConfig(iType)%x(iAtom) + 
     &                xOffset*(dble(NMAX(iType)/2d0 - dble(iMol)))
            ycm = ycm + gasConfig(iType)%y(iAtom) + 
     &                yOffset*dble(iType)
