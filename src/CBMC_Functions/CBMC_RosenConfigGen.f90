...
!      Begin the regrowth process by choosing an insertion site for the first atom in the chain
      ...
      do iRosen = 1, nRosenTrials(nType)
        ...
        write(2,*) iRosen, trialPos(iRosen)%x, trialPos(iRosen)%y ,trialPos(iRosen)%z, E_Trial(iRosen)
      enddo
      E_Max = minval(E_Trial)
      ProbRosen = 0d0
      do iRosen = 1, nRosenTrials(nType)
      ...
      rosenRatio = ProbRosen(nSel)/(rosenNorm*dble(nRosenTrials(nType)) )
      regrown(1) = .true.
...
!      the second atom must be inserted at the correct bond distance from the first atom.
      ...
      rosenRatio = rosenRatio*ProbRosen(nSel)/(rosenNorm*dble(nRosenTrials(nType)))
      regrown(Atm2) = .true.
...
!      Begin the regrowth process by choosing an insertion site for the first atom in the chain
      ...
      enddo
      rosenNorm = sum(ProbRosen)
      rosenRatio = ProbRosen(1)/(rosenNorm*dble(nRosenTrials(nType)))
      regrown(1) = .true.
...
!      the second atom must be inserted at the correct bond distance from the first atom.
      ...
      enddo
      rosenNorm = sum(ProbRosen)
      rosenRatio = rosenRatio*ProbRosen(1)/(rosenNorm*dble(nRosenTrials(nType)))
      regrown(Atm2) = .true.
